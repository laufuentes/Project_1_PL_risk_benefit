...
for (b in 1:B){
   ...
   res_files <- list.files(path = res_dir, pattern = pattern, full.names = TRUE)
    ...
    saveRDS(results$optimal_x[[idx_opt]], file = file.path("MC","results","oracular","psi_opt",paste0("opt_psi_",b,".rds")))
    saveRDS(results$lambda[[idx_opt]], file = file.path("MC","results","oracular", "lambda_opt",paste0("opt_lambda_",b,".rds")))
    saveRDS(results, file = file.path("MC","results","oracular", "group_by_ind",paste0("individual_",b,".rds")))
...
for (l in 1:L){
  ...
  data.frame(
    ...
    obj = x[[6]],
    policy_value = x[[7]])}))
    saveRDS(results, file = file.path("MC","results","oracular", "group_by_lambda",paste0("lambda_",l,".rds")))
    write.csv(results, file = file.path("MC","results","oracular", "group_by_lambda",paste0("lambda_",l,".rds")), row.names = FALSE)
}
