...
write.csv(df_complete,file.path("results","oracular","df_complete.csv"))
write.csv(df,file.path("results","estimated","df.csv"))


lambda <- seq(0,15,1) 
saveRDS(lambda, file = file.path("results","data","Lambda.rds"))

B <- c(0.05, 0.25,0.5,1,1.5,2)
saveRDS(B, file = file.path("results","data","B.rds"))

param_grid_or <- expand.grid(lambda=lambda, beta = B)
saveRDS(param_grid_or, file = file.path("results","data","grid_or.rds"))

Jfold<- 5
saveRDS(Jfold, file = file.path("results","data","Jfolds.rds"))

param_grid_est <- expand.grid(Fold = 1:Jfold, lambda = lambda, beta = B)
saveRDS(param_grid_est, file = file.path("results","data","grid_est.rds"))


source(file.path("src","estimation.R"))
s<- partition_data(Jfold, df)
saveRDS(s, file = file.path("results","data","s.rds"))
...
saveRDS(mu.hat.nj, file = file.path("results","data","mu.hat.nj.rds"))

nu.hat.nj <- initial_nparams[[3]]
saveRDS(nu.hat.nj, file = file.path("results","data","nu.hat.nj.rds"))
...
saveRDS(Delta_mu_nj_folds, file = file.path("results","data","Delta_mu_nj_folds.rds"))
saveRDS(Delta_nu_nj_folds, file = file.path("results","data","Delta_nu_nj_folds.rds"))
saveRDS(CC_mu, file = file.path("results","data","CC_mu.rds"))
saveRDS(CC_nu, file = file.path("results","data","CC_nu.rds"))
