...
df <- read.csv("opt_results/estimation_T/df.csv", stringsAsFactors=FALSE)
 ...
 res_dir <- "opt_results/estimation_T/indiv_res"
...
saveRDS(results, file = "opt_results/estimation_T/results.rds")
write.csv(results%>%select(-optimal_x), file = "opt_results/estimation_T/results.csv", row.names = FALSE)
...
write.csv(
  result_T,
  paste0("opt_results/estimation_T/result_T.csv")
)

saveRDS(result_T, file = "opt_results/estimation_T/result_T.rds")
...
ggsave(paste0("images/estimation_T/Optimal_policy.pdf"),opt_plots, width = 8, height = 4)
